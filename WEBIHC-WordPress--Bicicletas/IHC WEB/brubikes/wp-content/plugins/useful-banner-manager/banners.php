...
<div class="wrap">
    ...
    <h1>Useful Banner Manager</h1>
    <h2><?php _e( 'Banners', 'useful-banner-manager' ); ?></h2>
    <?php
    if ( isset( $_GET[ $ubm_plugin_prefix . 'banner_id' ] ) && is_numeric( $_GET[ $ubm_plugin_prefix . 'banner_id' ] ) && $_GET[ $ubm_plugin_prefix . 'banner_id' ] > 0 ) {
        if ( $_GET['page'] == 'useful-banner-manager/banners.php' ) {
            if ( isset( $_POST[ $ubm_plugin_prefix . 'save_banner' ] ) ) {
                $banner_id = $_GET[ $ubm_plugin_prefix . 'banner_id' ];

                $banner_old_name = $_POST[ $ubm_plugin_prefix . 'banner_name' ];
                $banner_old_type = $_POST[ $ubm_plugin_prefix . 'banner_type' ];
                $banner_old_file = $banner_id . '-' . $banner_old_name . '.' . $banner_old_type;

                $errors = array();

                if ( $_FILES[ $ubm_plugin_prefix . 'banner_file' ]['error'] == 0 ) {
                    $banner_name_parts = explode( '.', $_FILES[ $ubm_plugin_prefix . 'banner_file' ]['name'] );
                    array_pop( $banner_name_parts );

                    $banner_name = implode( '.', $banner_name_parts );

                    $banner_file_parts = explode( '.', $_FILES[ $ubm_plugin_prefix . 'banner_file' ]['name'] );
                ...
                $banner_alt = esc_attr( stripslashes( $_POST[ $ubm_plugin_prefix . 'banner_alt' ] ) );

                $banner_link = esc_url_raw( $_POST[ $ubm_plugin_prefix . 'banner_link' ] );

                if ( $banner_link == '' ) {
                    $link_target = '';
                } else {
                    if ( in_array( $_POST[ $ubm_plugin_prefix . 'link_target' ], array( '_self', '_top', '_blank', '_parent' ) ) ) {
                        $link_target = $_POST[ $ubm_plugin_prefix . 'link_target' ];
                    } else {
                        $link_target = '_self';
                    }
                }
                ...
                if ( $banner_link == '' ) {
                    ...
                    if ( in_array( $_POST[ $ubm_plugin_prefix . 'link_rel' ], array( 'nofollow', 'dofollow' ) ) ) {
                        $link_rel = $_POST[ $ubm_plugin_prefix . 'link_rel' ];
                    } else {
                        $link_rel = 'dofollow';
                    }
                }

                if ( isset( $_POST[ $ubm_plugin_prefix . 'auto_sizes' ] ) && ! in_array( 'banner_type', $errors ) ) {
                    if ( $banner_type == 'swf' ) {
                        $errors[] = 'swf_auto_sizes';
                    } else {
                        list( $banner_width, $banner_height ) = @ getimagesize( $banner_tmp_file );
                    }
                } elseif ( ! isset( $_POST[ $ubm_plugin_prefix . 'auto_sizes' ] ) ) {
                    if ( is_numeric( $_POST[ $ubm_plugin_prefix . 'banner_width' ] ) && $_POST[ $ubm_plugin_prefix . 'banner_width'] > 0 ) {
                        $banner_width = $_POST[ $ubm_plugin_prefix . 'banner_width' ];
                    } else {
                        $banner_width = '';
                    ...
                    if ( is_numeric( $_POST[ $ubm_plugin_prefix . 'banner_height' ] ) && $_POST[ $ubm_plugin_prefix . 'banner_height'] > 0 ) {
                        $banner_height = $_POST[ $ubm_plugin_prefix . 'banner_height' ];
                    } else {
                        $banner_height = '';
                ...
                if ( $_POST[ $ubm_plugin_prefix . 'active_until' ] == '' ) {
                    $active_until = -1;
                } else {
                    $active_until = trim( $_POST[ $ubm_plugin_prefix . 'active_until' ] );
                ...
                if ( is_numeric( $_POST[ $ubm_plugin_prefix . 'banner_order' ] ) && $_POST[ $ubm_plugin_prefix . 'banner_order' ] >= 0 ){
                    $banner_order = $_POST[$ubm_plugin_prefix . 'banner_order'];
                } else {
                    $errors[] = 'banner_order';
                }

                $wrapper_id = esc_attr( stripslashes( $_POST[ $ubm_plugin_prefix . 'wrapper_id' ] ) );
                $wrapper_class = esc_attr( stripslashes( $_POST[ $ubm_plugin_prefix . 'wrapper_class' ] ) );

                switch( $_POST[ $ubm_plugin_prefix . 'is_visible' ] ) {
                    case 'no':
                    $is_visible = 'no';
                    break;
                    default:
                ...
                if ( empty( $errors ) ) {
                    ...
                    ubm_update_banner( $banner_id, $banner_data );
        ...
        $banner_id = $_GET[ $ubm_plugin_prefix . 'banner_id' ];
        $banner = ubm_get_banner( $banner_id );
        ...
        if ( empty( $banner ) ) {
            ...
            <form method="post" enctype="multipart/form-data">
              <table id="ubm_edit_banner">
                  ...
                  <tr>
                      ...
                      <td width="75%">
                          ...
                          <table>
                              <tr>
                                <td><label for="<?php echo( $ubm_plugin_prefix ); ?>banner_width"><?php _e( 'Width:', 'useful-banner-manager' ); ?></label></td>
                                <td><input type="text" name="<?php echo( $ubm_plugin_prefix ); ?>banner_width" id="<?php echo( $ubm_plugin_prefix ); ?>banner_width" style="width: 50px" <?php if ( ! empty( $errors ) && ! isset( $_POST[ $ubm_plugin_prefix . 'auto_sizes' ] ) ) { echo( 'value="' . $banner_width . '"' ); } else { echo( 'value="' . ( empty( $banner->banner_width ) ? '' : $banner->banner_width ) . '"' ); } ?> /><?php _e( 'px', 'useful-banner-manager' ); ?> <?php _e( '(required if the banner is swf file)', 'useful-banner-manager' ); ?></td>
                              </tr>
                              <tr>
                                <td><label for="<?php echo( $ubm_plugin_prefix ); ?>banner_height"><?php _e( 'Height:', 'useful-banner-manager' ); ?></label></td>
                                <td><input type="text" name="<?php echo( $ubm_plugin_prefix ); ?>banner_height" id="<?php echo( $ubm_plugin_prefix ); ?>banner_height" style="width: 50px" <?php if ( ! empty( $errors ) &&  ! isset( $_POST[ $ubm_plugin_prefix . 'auto_sizes' ] ) ) { echo( 'value="' . $banner_height . '"' ); } else { echo( 'value="' . ( empty( $banner->banner_height ) ? '' : $banner->banner_height ) . '"' ); } ?> /><?php _e( 'px', 'useful-banner-manager' ); ?> <?php _e( '(required if the banner is swf file)', 'useful-banner-manager' ); ?></td>
                              </tr>
                          </table>
                      </td>
                  </tr>
                  ...
                  <tr>
                      ...
                      <td width="75%">
                          <label><?php _e( 'Yes:', 'useful-banner-manager' ); ?><input type="radio" name="<?php echo( $ubm_plugin_prefix ); ?>is_visible" value="yes" <?php if ( ! empty( $errors ) && $is_visible != 'no' ) { echo( 'checked="checked"' ); } elseif ( ( empty( $errors ) ) && $banner->is_visible == 'yes' ) { echo( 'checked="checked"' ); } ?> /></label> <label><?php _e( 'No:', 'useful-banner-manager' ); ?><input type="radio" name="<?php echo( $ubm_plugin_prefix ); ?>is_visible" value="no" <?php if ( ! empty( $errors ) && $is_visible == 'no' ) { echo( 'checked="checked"' ); } elseif ( ( empty( $errors ) ) && $banner->is_visible == 'no' ) { echo( 'checked="checked"' ); } ?> /></label>
                      </td>
                  </tr>
                  <tr>
                    <td colspan="2">&nbsp;</td>
              ...
              <p class="submit">
                  <input name="<?php echo( $ubm_plugin_prefix ); ?>save_banner" type="submit" class="button" value="<?php _e( 'Save', 'useful-banner-manager' ); ?>" /> <a href="admin.php?page=useful-banner-manager/banners.php"><?php _e( 'Cancel', 'useful-banner-manager' ); ?></a>
              </p>
          </form>
        ...
        if ( $_GET['page'] == 'useful-banner-manager/banners.php' ) {
            if ( isset( $_POST[ $ubm_plugin_prefix . 'add_banner' ] ) ) {
                $errors = array();
                ...
                if ( empty( $errors ) ) {
                    ...
                    $added_banner_id = ubm_add_banner( $banner_data );
            ...
            if ( isset( $_POST[ $ubm_plugin_prefix . 'delete' ] ) ) {
                if ( isset( $_POST[ $ubm_plugin_prefix . 'banner_id' ] ) && is_numeric( $_POST[ $ubm_plugin_prefix . 'banner_id' ] ) && $_POST[ $ubm_plugin_prefix . 'banner_id' ] > 0 ) {
                    $banner_id = $_POST[ $ubm_plugin_prefix . 'banner_id' ];

                    ubm_delete_banner( $banner_id );
        ...
        <form method="post" enctype="multipart/form-data">
          <table id="ubm_add_banner">
              ...
              <tr>
                  <td width="25%" valign="middle"><strong><?php _e( 'Banner Title', 'useful-banner-manager' ); ?></strong></td>
                  <td width="75%">
                      <input type="text" name="<?php echo( $ubm_plugin_prefix ); ?>banner_title" id="<?php echo( $ubm_plugin_prefix ); ?>banner_title" style="width: 300px" <?php if ( ! empty( $errors ) ){ echo( 'value="' . $banner_title . '"' ); } ?> /> <?php _e( '(required)', 'useful-banner-manager' ); ?>
                  </td>
              </tr>
              <tr>
                  <td width="25%" valign="middle"><strong><?php _e( 'Image Alt', 'useful-banner-manager' ); ?></strong></td>
              ...
              <tr>
                  ...
                  <td width="75%">
                      ...
                      <table>
                          <tr>
                            <td><label for="<?php echo( $ubm_plugin_prefix ); ?>banner_width"><?php _e( 'Width:', 'useful-banner-manager' ); ?></label></td>
                            <td><input type="text" name="<?php echo( $ubm_plugin_prefix ); ?>banner_width" id="<?php echo( $ubm_plugin_prefix ); ?>banner_width" style="width: 50px" <?php if ( ! empty( $errors ) && ! isset( $_POST[ $ubm_plugin_prefix . 'auto_sizes' ] ) ) { echo( 'value="' . $banner_width . '"' ); } ?> /><?php _e( 'px', 'useful-banner-manager' ); ?> <?php _e( '(required if the banner is swf file)', 'useful-banner-manager' ); ?></td>
                          </tr>
                          <tr>
                            <td><label for="<?php echo( $ubm_plugin_prefix ); ?>banner_height"><?php _e( 'Height:', 'useful-banner-manager' ); ?></label></td>
                            <td><input type="text" name="<?php echo( $ubm_plugin_prefix ); ?>banner_height" id="<?php echo( $ubm_plugin_prefix ); ?>banner_height" style="width: 50px" <?php if ( ! empty( $errors ) && ! isset( $_POST[ $ubm_plugin_prefix . 'auto_sizes' ] ) ) { echo( 'value="' . $banner_height . '"' ); } ?> /><?php _e( 'px', 'useful-banner-manager' ); ?> <?php _e( '(required if the banner is swf file)', 'useful-banner-manager' ); ?></td>
                          </tr>
                      </table>
                  </td>
              </tr>
              <tr>
                  <td width="25%" valign="middle"><strong><?php _e( 'Active Until', 'useful-banner-manager' ); ?></strong></td>
                  <td width="75%">
                      <input type="text" name="<?php echo( $ubm_plugin_prefix ); ?>active_until" id="<?php echo( $ubm_plugin_prefix ); ?>active_until" class="datepicker" style="width: 100px" <?php if ( ! empty( $errors ) ) { if( in_array( 'active_until', $errors ) ) { echo( 'value="' . esc_attr( $_POST[ $ubm_plugin_prefix . 'active_until' ] ) . '"'); } elseif ( $active_until != -1 ) { echo( 'value="' . $active_until . '"' ); } } ?> /> <small><?php _e( 'Leave empty if there is no date.', 'useful-banner-manager' ); ?></small>
                  </td>
              </tr>
              <tr>
                  <td width="25%" valign="middle"><strong><?php _e( 'Banner Order', 'useful-banner-manager' ); ?></strong></td>
          ...
          <p class="submit">
              <input name="<?php echo( $ubm_plugin_prefix ); ?>add_banner" type="submit" class="button" value="<?php _e( 'Add banner', 'useful-banner-manager' ); ?>" />
          </p>
        </form>
        <script type="text/javascript">
        if(jQuery.isFunction(jQuery.fn.datepicker)){
        ...
        <table class="widefat fixed" cellspacing="0" id="ubm_manage_banners" width="100%">
              <thead>
              	<tr>
                    ...
                    <td>
                      <form method="get">
                          <p class="submit">
                            ...
                            <input type="hidden" name="page" value="useful-banner-manager/banners.php" />
                            <input type="hidden" name="<?php echo( $ubm_plugin_prefix ); ?>banner_id" value="<?php echo( $banner->id ); ?>" />
                          </p>
                      </form>
                    </td>
                    <td>
                      <form method="post">
                          <p class="submit">
                            <input name="<?php echo( $ubm_plugin_prefix ); ?>delete" type="submit" class="button" value="<?php _e( 'Delete', 'useful-banner-manager' ); ?>" onclick="javascript:if(!confirm('<?php echo( sprintf( __( 'Are you sure you want to delete the banner &quot;%s&quot;?', 'useful-banner-manager' ), $banner->banner_title ) ); ?>')){ return false; }" />
                            <input type="hidden" name="<?php echo( $ubm_plugin_prefix ); ?>banner_id" value="<?php echo( $banner->id ); ?>" />
                          </p>
                      </form>
                    </td>
